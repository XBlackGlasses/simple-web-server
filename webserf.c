...
#include <signal.h>
#include <fcntl.h>
#define PORT "2000" // 提供給使用者連線的 port
...
int main(void)
{
	...
	if(setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(yes)) == -1) //有些連接到 socket 的連線還懸在 kernel 裡面，而它佔據了這個 port
    {
        perror("setsockopt");
        exit(1);
    }
	...
	sigemptyset(&sa.sa_mask);   //會被阻止的signal
	sa.sa_flags = SA_RESTART;

	sigaction(SIGCHLD, &sa, NULL);	//處理zombie
